...
-- {{{ Init cairo-compmgr, pulseaudio, fcitx
awful.util.spawn_with_shell("cairo-compmgr &")
awful.util.spawn_with_shell("run_once pulseaudio --start")
...
-- The default is dark theme (more to see "path-to-awesome/themes/")
...
mymainmenu = awful.menu({ items = { { "awesome", myawesomemenu, beautiful.awesome_icon },
                                    { "Terminal", terminal, '/usr/share/icons/Faenza/apps/16/gnome-terminal.png' },
                                    { "Gvim", "gvim", '/usr/share/icons/Faenza/apps/16/gvim.png' },
                                    { "Nemo", "nemo", '/usr/share/icons/Faenza/apps/16/file-manager.png'},
                                    { "Chromium", "chromium", '/usr/share/icons/Faenza/apps/16/chromium.png' },
                                    { "Util", xdgmenu},
...
mytextclock = awful.widget.textclock(" %H:%M %A ", 1)
...
function update_netstat()
  ...
  if netif then
    ...
    netdata = {} -- clear for reset interface
    text = '{No network}'
  end
...
function update_batwidget()
  ...
  local line = f:read("*l")
  f:close()
  local state, percent = string.match(line, '^Battery%s+%d+:%s+(%w+),%s+(%d+)%%')

  state = battery_stata[state] or battery_stata.unknown
  if tonumber(percent) <= 35 then
...
-- {{{ Volume Controller using pamixer(from aur)
...
awful.util.spawn_with_shell("/usr/lib/polkit-1/polkitd --no-debug")
